...
question_inference_mapping = {
    "ara": {"uses": {"علب الصفيح": "ما هو الاستخدام المفاجئ لـ علب الصفيح؟"}},
    "chi": {
        "uses": {
            "光盘": "光盘有什么令人惊讶的用途？",
            "冰块": "冰块有什么令人惊讶的用途？",
...
def infer_question(prompt, language="eng"):
    ...
    if (
        language in question_inference_mapping
        and prompt.upper() in question_inference_mapping[language]["uses"]
    ):
...
def simple_stats(df, rater_cols=False, name=None, save_dir=None):
    ...
    if rater_cols and len(rater_cols) > 1:
        try:
            x = df[["id"] + rater_cols].melt(
                id_vars="id", var_name="rater", value_name="rating"
            )
            ...
            stats_data["ICC2k_CI"] = (
                str(stats_data["ICC2k_CI"][0]) + "-" + str(stats_data["ICC2k_CI"][1])
            )
    ...
    if save_dir:
        ...
        conn.sql(
            "CREATE TABLE IF NOT EXISTS stats (name TEXT, no_of_prompts INTEGER, "
        ...
        existing_rows = conn.sql(
            f"SELECT * FROM stats WHERE name = '{name}'"
        ).fetchall()
        if existing_rows:
...
def prep_general(
    ...
    if question_mappings:
        ...
        assert set(question_mappings.keys()) == set(
            data.prompt.unique()
        ), "There are prompts that don't have a question mapping"
    ...
    for col in rater_cols:
        clipped_col = data[col].apply(
            lambda x: x if range[0] <= x <= range[1] else np.nan
        )
        # if there are any clipped values, print a warning
        if clipped_col.isna().sum() > data[col].isna().sum():
            mprint(
                f"WARNING: {clipped_col.isna().sum()} out-of-range values clipped from {col}"
            )
    ...
    if round_adjust:
        ...
        data["avg_rating"] = data.avg_rating + (
            data.median_rating - data.avg_rating
        ).div(10**3)
    if include_rater_std:
        data["rating_std"] = data[rater_cols].std(1, numeric_only=True, skipna=True)

    data["target"] = normalize_values(
        data.avg_rating, oldrange=range, round_precision=round_precision
    )
    ...
    if ("question" not in data.columns) or overwrite_q:
        try:
            ...
            raise Exception(
                "can't infer question format for anything other "
...
def combine_dupes(df):
    ...
    if len(df) == 1:
        ...
        if (
            ("rating_std" in df.columns)
            and ("rater_count" in df.columns)
            and (df.rating_std.isna().sum() == 0)
        ):
            sum_of_squares = df.apply(
                lambda row: (row.rater_count - 1) * row.rating_std**2, axis=1
            ).sum()
            outdict["rating_std"] = np.sqrt(
                sum_of_squares / (outdict["rater_count"] - len(df))
            )
        else:
...
def normalize_values(series, outrange=(1, 5), oldrange=None, round_precision=None):
    """
