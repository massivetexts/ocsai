        ...
        if extract_files and extension == "zip":
            for extract_file in extract_files:
                ...
                if not extract_file_path.exists():
                    raise Exception(
                        f"File {extract_file_path} not found in {file_path}"
                    )
...
def load_dataset(dataset, data_dir, verbose=True, include_rater_std=True):
    ...
    prepped_data = prep_general(
        data, name, original_range=original_range, include_rater_std=include_rater_std
    )
    return prepped_data


def load_dataset_definitions(
    definition_file, data_dir, verbose=True, include_rater_std=False
):
    ...
    datasets = {}
    for dataset_def in tqdm(datasets_definition, desc="Processing datasets"):
        datasets[dataset_def["name"]] = load_dataset(
            dataset_def, data_dir, verbose=verbose, include_rater_std=include_rater_std
        )
