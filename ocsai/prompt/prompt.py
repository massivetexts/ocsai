...
from ocsai.parsers.numbered_list_output_parser import NumberedListOutputParser
from ..parsers.scored_csv_output_parser import ScoredCSVOutputParser
...
class PromptScorer:
    def __init__(
        ...
        if openai_key_path:
            ...
            openai_api_key = None  # fall back on thei openai flow
    ...
    def originality(
        ...
        return_prompt: bool | None = False,
        style: int | PromptTemplate = 1,
    ) -> float:
    ...
    def _generic_scorer(
        ...
        style: str | int | PromptTemplate = 1,
    ) -> float:

        if not prompt and not full_prompt and "mixed" not in task:
            raise ValueError(
                "Either prompt or full_prompt must be provided, unless using 'mixed' task types (e.g. 'uses_mixed')"
            )
        ...
        if "splitlist" in style:
            ...
            template_args["example_uses"] = "\n".join(
                [f'{i+1}. {ex["response"]}' for i, ex in enumerate(examples)]
            )
            template_args["example_scores"] = "\n".join(
                [f'{i+1}. {ex["score"]}' for i, ex in enumerate(examples)]
            )
            if type(response) is str:
                response = [response]
            template_args["responses"] = "\n".join(
                [
                    f"{i+1+len(examples)}. {response}"
        ...
        if parse:
            if "splitlist" not in style:
                template_args["format_instructions"] = (
    ...
    def response_formatter(
        self, response: str | list[str], number_response: bool = False
    ):
...
class DatasetPromptScorer:
    ...
    def __init__(
        ...
        if self.type_col not in data:
            if not testtype:
                raise KeyError(
                    f"If you don't have a {self.type_col} column in your data, you need to specify your testtype (for the whole dataset) with the `testtype` arg"
                )
        ...
        if self.prompt_col not in data:
            if not prompt:
                raise KeyError(
                    f"If you don't have a {self.prompt_col} column in your data, you need to specify your prompt (for the whole dataset) with the `prompt` arg"
                )
            data[type_col] = testtype
        ...
        for col in [self.response_col, self.score_col]:
            if col not in data.columns:
                raise KeyError(
                    f"Your input data needs columns specifying testtype, prompt (the item), id (a unique id), response, and score (for the train data). Missing '{col}' column."
                )
        ...
        if self.id_col not in data:
            ...
            data[id_col] = (
                data[type_col] + data[prompt_col] + data[response_col]
            ).apply(compute_hash)
    ...
    def split_data(self, data):
        ...
        train_val, test = train_test_split(
            data, test_size=1 - self.train_prop, random_state=self.seed
        )
        relative_val_prop = self.val_prop / (1 - (1 - self.train_prop))
        train, val = train_test_split(
            train_val, test_size=relative_val_prop, random_state=self.seed
        )
    ...
    def score(
        ...
        ticks_per_run = (
            (self.testdata.groupby(["type", "prompt"]).size() / n_per_prompt)
            .apply(np.ceil)
            .sum()
        )
        ...
        for run_n in range(repeat):
            for (testtype, prompt), type_subset in self.testdata.groupby(
                ["type", "prompt"]
            ):
                chunks = self._split_dataframe(type_subset, n_per_prompt)
                for i, chunk in enumerate(chunks):
                    pbar.set_description(
                        f"Run {run_n+1}: Processing {testtype}/{prompt} - chunk {i+1}"
                    )
                    to_score = chunk.response.str.strip().tolist()
                    example_pool = self.traindata[
                        self.traindata[self.prompt_col] == prompt
                    ]
                    examples_dict, example_ids = self.get_examples(
                        n_examples, example_pool, example_strategy
                    )
                    ...
                    results_df = pd.DataFrame(
                        [
                            (testtype, prompt, response, prediction)
                            for response, prediction in results
                        ],
                        columns=[
                            self.type_col,
                            self.prompt_col,
                    ...
                    if len(merged_df) != len(chunk):
                        if len(results_df) == len(chunk):
                            logging.debug(
                                "Join failed - trying simply to align columns"
                            )
                            merged_df = chunk.copy()
                            merged_df[["response_check", "prediction"]] = results
                        else:
                            logging.warning(
                                "Second fallback - left join - some scores may be missed"
                            )
                    ...
                    if fifty_scale:
                        ...
                        merged_df["prediction"] = merged_df["prediction"].apply(
                            divide_if_possible
                        )
                    # add metadata
                    ...
                    if save_location:
                        pd.concat(all_results).to_csv(
                            Path(save_location) / fname, index=False
                        )
    ...
    def get_examples(self, n, data=None, strategy="random", include_prompt=False):
        ...
        if strategy == "random":
            n = min(n, len(data))
            examples = data.sample(n)[
                [self.id_col, self.prompt_col, self.response_col, self.score_col]
            ]
        else:
            raise ValueError(f'strategy="{strategy}" is not implemented')
        if include_prompt:
            examples_dict = (
                examples[[self.prompt_col, self.response_col, self.score_col]]
                .rename(
                    columns={
                        self.prompt_col: "item",
            ...
            examples_dict = (
                examples[[self.response_col, self.score_col]]
                .rename(
                    columns={self.response_col: "response", self.score_col: "score"}
                )
                .to_dict(orient="records")
            )
