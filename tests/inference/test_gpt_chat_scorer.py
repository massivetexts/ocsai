...
def test_chat_scorer_score_method_ocsai15():
    ...
    result = chat_scorer.score("Pants", "makeshift flag")
    assert result == {"score": 3.0, "confidence": 2, "flags": None}
...
# Test crafting prompt
def test_chat_scorer_craft_prompt_ocsai():
    chat_scorer = GPT_Chat_Scorer(
        cache=None, model_dict=ocsai15_chat_models, prompter=GPT_Ocsai2_Prompter()
    )
    ans = """ACTION: TAG THE ORIGINALITY OF A RESPONSE TO A CREATIVITY TEST.
TASK: uses
PROMPT: Pants
LANGUAGE: eng
RESPONSE: `makeshift flag`
...
- `FLAGS`: A comma-separated list with content flags, such as: 'nonsense', 'violent', 'not practical' """.strip()
    prompt = chat_scorer.prompter.craft_prompt("Pants", "makeshift flag")
    assert prompt == ans.strip()
