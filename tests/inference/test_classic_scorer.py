from ocsai.inference.classic_scorer import Classic_Scorer
import pandas as pd
...
def test_classic_scorer_initialization():
    scorer = Classic_Scorer(cache=None, model_dict=ocsai1_classic_models)
...
def test_originality_batch_method():
    scorer = Classic_Scorer(cache=None, model_dict=ocsai1_classic_models)
    results = scorer.originality_batch(['Pants', 'Pants'], ['makeshift flag', 'make a parachute'],
                                       model='ocsai-davinci2')
    expected_results = [{'score': 3.3, 'confidence': None, 'flags': None},
                        {'score': 2.7, 'confidence': None, 'flags': None}]
...
def test_originality_batch_with_questions():
    scorer = Classic_Scorer(cache=None, model_dict=ocsai1_classic_models)
    try:
        results = scorer.originality_batch(prompts=[],  # noqa: F841
...
def test_originality_df_method():
    scorer = Classic_Scorer(cache=None, model_dict=ocsai1_classic_models)
    df = pd.DataFrame([['Pants', 'makeshift flag'], ['Pants', 'make a parachute']],
                      columns=['prompt_randname', 'response_randname'])
    scored_df = scorer.originality_df(df, model='ocsai-davinci2',
                                      prompt_col='prompt_randname',
                                      response_col='response_randname')
    expected_dict = {
